...
## 9. E2E 实测记录：验证到了什么、没验证到什么

2026-09-22 在真实服务上跑（后端 `:3188` 重启后跑新代码，前端 vite `:5188`），用**临时调度 +
临时任务**做的实验，跑完已全部清理（0 条残留）。

### 验证到的

1. **后端守卫真的会拒绝**：上一轮运行中时 `POST /api/scheduled-tasks/:id/run-now` →
   **409 + `code: 'SCHEDULE_RUNNING'` + `details.taskId`**（真实 curl，非 mock）。
2. **并发只建一条任务**：2 发并发 → 1×200 + 1×409；10 发并发 → 1×200 + 9×409，
   查库确认任务数只 +1。
3. **UI 禁用态**（桌面表格与移动卡片两套布局都测）：被阻断的行 `disabled=true`、
   颜色 `rgba(106,113,129,0.5)`（灰）、`title="上一轮还在运行中，先等它结束或中断它"`；
   未被阻断的行 `disabled=false`、`rgb(3,105,160)`（info 蓝）、`title="立即触发"`。
4. **错误条**（故障注入 409，见下）：点击后渲染、文案逐字正确、位于列表**上方**、
   点「关闭」后消失；点击前不存在。
5. **放行路径**：上一轮 settled（`in_review`）后按钮恢复可点，`run-now` 成功建出任务。

### 没验证到的（以及一个值得记的发现）

1. **在途闸门在真实并发下一次都没命中。** 10 发并发里 9 条拒绝全部来自状态守卫
   （消息是 `unfinished run` 而非 `is still dispatching`）。原因：对**已有标题**的调度，
   `dispatch` 几乎是全同步的 —— `createTask` 不碰 LLM、DB 是 better-sqlite3 同步写、
   `startTaskRun` 同步 kick off，所以它的窗口在微秒级，两个 HTTP 请求不可能重叠。
   **结论：这道闸门目前是防御性的**，只有当 `dispatch` 将来真的 `await` 一个会 yield 到
   事件循环的操作时才会变成承重的。它的机制由单测钉住（故意挂起 `createTask`）。
2. **空标题拉不宽派发窗口**：标题在**保存时**就由 LLM 解析并落库（`create` 里的
   `resolveGeneratedTitle`），派发时 `createTask` 拿到的已经是标题。所以「legacy 空标题行」
   这条触发路径在生产上也到不了。
3. **错误条是故障注入验的，不是自然撞出来的。** 要自然撞上 409，得让前端看不见后端看见的
   东西（`isSessionRunning`），而那需要「人工把一个正在跑的任务标成 done」——为不碰真实业务
   数据没做。渲染本身另有静态标记测试覆盖，触发路径另有 `runNowErrorMessage` 单测覆盖。
4. **`waiting_answer` / `waiting_plan` / `waiting_approval` 三个 title 文案分支没在真实浏览器里
   观察到**（单测覆盖了全部分支）。试过用「需要审批的工具调用」把任务钉在 `waiting_approval`，
   但 agent 没真的调工具、很快就结束了。

### 对真实数据的影响

实验全程只动自己建的临时调度与任务；用户那条真实业务调度「同步付款审批并通知」未被触碰
（它在实验窗口内被自己的 30 分钟 tick 正常触发并跑完，与本次改动无关）。清理后查库确认
无残留。库里另有 3 条孤儿任务（`source_schedule_id` 指向已删调度），是 2026-08 的历史遗留，
不是本次产生。
