...
fontSize: {
  ...
  "3xs": "10px",
  "2xs": "11px",
  // 这五档是 Tailwind 自带的默认值，写出来只为让刻度表在一处可见。
  // 必须保留 [字号, 行高] 元组形式与 rem 单位——理由见下。
...
1. **`xs`/`sm`/`base`/`lg`/`xl` 必须写成 `[字号, { lineHeight }]` 元组，不能用裸字符串。**
   Tailwind 默认刻度每档都带行高，裸字符串会**把行高整个丢掉**，元素回落到 `html { line-height: 1.5 }`。实测影响约 780 处（`text-xs` 行高 16→18px、`text-sm` 20→21px 等）。

   > **本文档早期版本在这里写错了**：原文声称"当前用法都未指定行高，所以设行高会改观感"。这个理由**是反的**——源码里确实没有 `leading-*` 类，但**框架默认刻度一直在提供行高**。该错误由代码审查通过对比新旧 config 生成的 CSS 发现。

2. **这五档必须用 `rem`，不能用 `px`。**
   默认根字号下两者渲染相同，但 `rem` 会跟随用户的浏览器字号偏好，`px` 不会。改成 `px` 等于悄悄取消了这个可访问性行为。

**新增档位（`4xs`/`3xs`/`2xs`）保持裸字符串与 `px`**——它们替换的是 `text-[11px]` 这类任意值，本来就没有行高、本来就是 px，裸写才是等价的。`inline-code` 保持 `em`，同理。
