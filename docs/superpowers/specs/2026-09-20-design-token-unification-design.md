...
| 裸 Tailwind 调色板类名出现次数（`text-gray-400`、`bg-blue-900` …） | **1720** |
| 受影响文件数 | **99**（占 27%） |
...
| 暗色语义色 base | **与亮色相同**，仅 foreground 翻转 | 已验证在两种底色上对比度均达标；减少一半 token |
| 暗色主色前景 | 深色 `222 47% 8%`（非白色） | 亮蓝 `#3b82f6` 上白字仅 3.1:1，不达 AA；深色前景达 8.6:1 |
...
> 语义色 base 两态相同。已验证 `text-success`(#15803d) 在亮底为 5.0:1、在暗底为 5.8:1，均达 WCAG AA。
...
| 语义 | base | hex | foreground | 对比度 | 典型用途 |
|---|---|---|---|---|---|
| success | `142 72% 29%` | `#15803d` | `#ffffff` | 5.0:1 | 任务成功、连接正常 |
| warning | `32 95% 44%` | `#d97706` | `#281601` | 5.5:1 | 进行中、即将超时 |
| info | `201 96% 32%` | `#0369a1` | `#ffffff` | 5.9:1 | 提示、模型信息 |
| destructive | `0 72% 51%` | `#dc2626` | `#ffffff` | 4.8:1 | 失败、删除 |
...
--chart-4: 239 84% 67%;   /* #6366f1 */
--chart-5: 84 81% 44%;    /* #84cc16 */
--chart-6: 258 90% 66%;   /* #8b5cf6 */
--chart-7: 330 81% 60%;   /* #ec4899 */
--chart-8: 0 84% 60%;     /* #ef4444 */
--chart-9: 38 92% 50%;    /* #f59e0b */
--chart-10: 25 95% 53%;   /* #f97316 */
```
...
**硬编码 hex**（111 处）逐处处理：`#1c3fa8`/`#1a2d5c`/`#2f5fe0`/`#5b8cff`/`#0ea5e9`/`#6366f1` → `hsl(var(--primary))`；`editorStyles.ts` 的 20+ 处按 §6.3 改为 `var(--*)`；`stats` 的 10 色按 §6.1 提升为 `--chart-*`；其余按语义归类或移入豁免区。

## 8. 验收标准

基线（2026-09-20 实测）：裸色 **1720** 处 / **99** 文件；`dark:` 裸色配对 **579**；`.ts`/`.tsx` 硬编码 hex **111**；
...
3. `.ts`/`.tsx` 中硬编码 hex **为 0**，豁免区（`llm-logo-provider`、`components/terminal`）除外（基线 111）。
4. 亮/暗两态下，正文文字对背景对比度 ≥ 4.5:1，大字号 ≥ 3:1。
5. 视觉回归：任务板、聊天、侧边栏、设置四个主场景在亮暗两态下逐屏核对。
6. `npm run typecheck` 保持 **0 error**；`npm run lint` 保持 **0 error**（warning 数不高于 225）。
