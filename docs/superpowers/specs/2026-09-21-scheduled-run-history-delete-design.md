...
## 8. 实现与验收结果（2026-09-22）

**实现**：5 个提交，**后端零改动**（逐个提交核对 `git show --stat` 无 `backend/` 文件）。

| 提交 | 内容 |
|---|---|
| `6809137` | 新建 `runHistoryDelete.ts` + 单测（`DeleteOutcome` / `selectableRuns` / `toggleSelectAll` / `deleteOutcomeMessage`） |
| `e67019f` | 补「陈旧 id 会被丢弃」的测试 + 收紧两处 JSDoc |
| `992fc2a` | 视图勾选/删除 UI + 面板 `deleteRuns` + TaskBoard 兜底（4 文件一次提交） |
| `8edadaa` | 按**可选集**剪枝；选中集改为「只留失败的」；确认文案与选中规则抽成纯函数 |
| `71dd8fc` | 选中集规则改为「减去已删的」（行级删除不再误伤其它勾选）+ catch 补 `console.error` + 注释收敛 |
...
| eslint 逐文件 | 视图 3 / 视图测试 0 / 面板 1 / TaskBoard 0 / 两个新文件 0 —— 与改动前基线**逐项相同，零新增** |
| web 单测（9 个文件） | **129 pass / 0 fail** |

**浏览器 E2E**（puppeteer-core + 缓存 chromium 连 `:5188`）：**15/15 通过**。用**合成数据**（标题带 `[E2E-DELETE]`，直插 SQLite）而非真实记录，跑完已清理干净。

覆盖到的点：`in_progress` 行**没有**勾选框且删除按钮 `disabled` + `title="运行中，先停止再删除"`；单条删除弹出 confirm 且文案含「其关联会话也会一并删除」，确认后行消失、结果条「已删除 1 条」；勾选 2 条后操作条显示「已选 2 项」，批量弹窗文案含条数与会话承诺，确认后两行消失、结果条「已删除 2 条」；**刷新页面后被删的三条确实不在了**（证明不是只从 DOM 移除）；全选只选中可选行（实测 全选 26 = 可选行 26，总行 27 —— 那条 `in_progress` 被正确排除）；窄屏 900px 卡片渲染勾选框（52 = 26×2）。

**会话级联已实证**：删除前 `sessions` 表里存在的 3 个 e2e 会话，删除后查库计数为 **0** —— 这是本次唯一能证明「语义正确」而非「看起来对」的检查，已通过。

**过程中修掉的三个真问题**（都由审查发现，非计划预见）：

1. **选中集可残留已不可选的行**：用户勾选后某行被 WS 更新成 `in_progress`，勾选框消失却仍在「已选 N 项」里计数，且必然 409。剪枝改为按 `selectableRuns(runs)` 而非全部 `runs`。
2. **部分失败时把失败项的选中也清掉了**（**计划文本自身的错误**，与本文档 §4 冲突）：spec 要求「成功的行消失、失败的行仍在（选中集剪掉已删的）」，即失败项应保持选中以便重试。改为 `selectionAfterOutcome` 纯函数。
3. **行级删除误伤其它勾选**：选了 A、B 后点 C 那一行的删除，选中集会变成 `{C}`（或空），A/B 静默丢失。规则最终定为**「从原选中集里减去这次成功删掉的」** —— 对批量与行级两条路径都正确，且比原规则更简单。

**⚠️ E2E 过程中的事故（已修复，如实记录）**：第一版 E2E 脚本用「全局按文本找『删除』按钮」定位批量删除按钮，而每一行也有一个「删除」按钮，结果点到了表格**最后一行的行级删除**，误删了一条**真实**任务：`b74f7c12-…`（`smoke-test-once`，状态 `done`，8-14 创建的老 smoke 测试任务）。它的 `session_id` 指向的会话早在 9-20 之前就已被清理（不在 9-20 的备份里），所以没有对话记录损失。该行已从 `new-auth.db.bak-oneshot-verdict-20260920-141037` 完整还原（逐列比对，当前表仅多一个带默认值的 `auto_approve`），并已通过 API 确认可见。脚本已修正为**只在批量操作条内定位**删除按钮，重跑 15/15 通过。教训：对用户真实库做破坏性 E2E 时，选择器必须限定作用域，且优先用合成数据。
