...
5. **§2.6 的消费链路没有单测**（最终评审标为唯一"既承重、又完全无防护"的缺口）。`pendingCreatedId` / `hiddenCreated` / `filterStillHidesNewTask` 三个标识符只出现在 `TaskBoard.tsx`（该文件没有测试文件），`createdTaskHandoff.test.ts` 只覆盖纯读写函数；本 spec §5 与 plan Task 6 的 E2E 清单原本也只断言**可见**路径 —— 而"新任务被存下的筛选藏住时弹提示条"恰恰是 §2.6 存在的全部理由。已在 plan Task 6 清单补入该场景（预置收窄筛选 → 侧栏建任务 → 断言提示条出现 → 清除筛选后任务可见）。**仍无自动化防护**：`react-hooks` 时序（等 `loading` 落地、`navigate` 的顺序）不是纯计算，抽纯函数只能钉住"找不到也消费"这一半，所以这条依赖手工 E2E。
