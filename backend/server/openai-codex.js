...
function readCodexItemText(item) {
...
function readCodexItemStreamKey(item) {
  if (!item || typeof item !== 'object') {
    return null;
  }
...
function extractCodexStreamingUpdate(event, streamState) {
  ...
  const streamKey = readCodexItemStreamKey(item);
  if (!streamKey) {
    return null;
  }

  const nextText = readCodexItemText(item);
...
export async function queryCodex(command, options = {}, ws) {
  ...
  try {
    // CODEX_PATH_OVERRIDE: use an externally installed Codex (npm/pnpm global,
    // brew, official installer, ...). Default new Codex() spawns the binary
    // bundled inside @openai/codex's platform optional dep and ignores any
    // `codex` on PATH. The SDK only honors the path via the codexPathOverride
    // option — a bare string is silently ignored and falls back to bundled.
    codex = new Codex(
    ...
    for await (const event of streamedTurn.events) {
      ...
      const streamingUpdate = extractCodexStreamingUpdate(event, streamedAgentMessages);
