 ...
 * Lovdex CLI
 *
 * Provides command-line utilities for managing Lovdex
 *
 * Commands:
...
// Get the database path from app.config.json (same source as connection.ts).
function getDatabasePath() {
    return appConfig().get().database.path;
}

// Apply an explicit CLI --port (or legacy PORT) into app.config before index.js
// boots. index.js no longer reads SERVER_PORT from env (Task 7), so writing the
// env var would be silently ignored — persist it to config instead.
function applyCliPortOverride(serverPort, legacyPort) {
...
function showStatus() {
    ...
    // Configuration (sourced from app.config.json, not env)
    const cfg = appConfig().get();
    console.log(`\n${c.info('[INFO]')} Configuration:`);
    console.log(`       server.port: ${c.bright(String(cfg.server.port))}`);
    console.log(`       database.path: ${c.dim(cfg.database.path)}`);
    console.log(`       providers.claude.cliPath: ${c.dim(cfg.providers.claude.cliPath || 'claude (default)')}`);
    console.log(`       server.contextWindow: ${c.dim(cfg.server.contextWindow != null ? String(cfg.server.contextWindow) : '(sdk default)')}`);
    ...
    // Config file location (app.config.json is the single source of truth)
    const configFilePath = appConfig().filePath;
    const configExists = fs.existsSync(configFilePath);
    console.log(`\n${c.info('[INFO]')} Configuration File:`);
    console.log(`       ${c.dim(configFilePath)}`);
    console.log(`       Status: ${configExists ? c.ok('[OK] Exists') : c.warn('[WARN] Not found (using defaults)')}`);
    ...
    console.log(`      ${c.dim('>')} Access the UI at http://localhost:${cfg.server.port}\n`);
}
...
Commands:
  start            Start the Lovdex server (default)
  sandbox          Manage Docker sandbox environments
...
Options:
  -p, --port <port>           Set server port (default: 3188)
...
function showSandboxHelp() {
    console.log(`
${c.bright('Lovdex Sandbox')} — Run Lovdex inside Docker Sandboxes
...
Subcommands:
  ...
  ${c.bright('logs')}         Show Lovdex server logs
...
async function sandboxCommand(args) {
    ...
    switch (opts.subcommand) {
        ...
        case 'start': {
            ...
            console.log(`${c.info('▶')} Launching Lovdex web server...`);
        ...
        case 'create': {
            ...
            // Step 3: Start Lovdex inside the sandbox
            console.log(`${c.info('▶')} Launching Lovdex web server...`);
            sbx(['exec', opts.name, 'bash', '-c', 'nohup cloudcli start --port 3001 > /tmp/cloudcli-ui.log 2>&1 & disown']);
...
async function main() {
    ...
    // Persist CLI overrides into app.config.json — index.js/connection.ts read
    // config, not env, so writing env vars here would be silently ignored.
    applyCliPortOverride(options.serverPort, process.env.PORT);
    if (options.databasePath) {
        appConfig().update({ database: { path: options.databasePath } });
    }
