...
function dbg(line: string): void {
  try {
    appendFileSync('/tmp/chat-debug.log', `${new Date().toISOString()} ${line}\n`);
  } catch {
    // ignore
  }
}
...
async function handleChatSend(
  ...
  dbg(`[chat.send] sessionId=${sessionId} content=${JSON.stringify((data.content ?? '').toString()).slice(0, 80)}`);
  if (!sessionId) {
    dbg('[chat.send] -> SESSION_ID_REQUIRED');
  ...
  if (!session) {
    dbg(`[chat.send] -> SESSION_NOT_FOUND for ${sessionId}`);
    sendProtocolError(
      ws,
      'SESSION_NOT_FOUND',
  ...
  if (!spawnFn) {
    dbg(`[chat.send] -> UNSUPPORTED_PROVIDER ${provider}`);
    sendProtocolError(ws, 'UNSUPPORTED_PROVIDER', `Provider "${provider}" is not available.`, sessionId);
    return;
  }

  dbg(`[chat.send] session ok provider=${provider} project_path=${session.project_path} provider_session_id=${session.provider_session_id}`);
  ...
  if (!run) {
    dbg(`[chat.send] -> RUN_IN_PROGRESS for ${sessionId}`);
    sendProtocolError(
      ws,
      'RUN_IN_PROGRESS',
  ...
  try {
    ...
    dbg(`[chat.send] spawnFn resolved for ${sessionId}`);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    dbg(`[chat.send] spawnFn THREW for ${sessionId}: ${message}`);
...
export function handleChatConnection(
  ...
  ws.on('message', async (rawMessage) => {
    try {
      ...
      dbg(`[ws-in] type=${messageType} sessionId=${typeof data.sessionId === 'string' ? data.sessionId : '-'}`);
