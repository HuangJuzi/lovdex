 ...
 * providerSessionId and, after a tick, pushes the scripted `session:<appId>`
 * events. On `approval/respond` it records the request. Set `pushComplete=false`
 * to keep the run open (approval flow); the caller pushes `complete` later.
 */
function fakeLite(
  ws: WebSocket,
  opts: {
  ...
  ws.on('message', (raw) => {
    ...
    // Any other rpc: ack so nothing hangs.
    ws.send(JSON.stringify({ type: 'rpc_res', id: frame.id, ok: true, data: {} }));
  });
...
test('loopback lite: spawn → early resolve → events → terminal complete', async () => {
  ...
  try {
    ...
    fakeLite(ws, {
      ...
      scripted: [{ type: 'assistant', session_id: 'P1', message: { role: 'assistant' } }],
    });
    ...
    await waitFor(() => registry.getSessionHost('s1') === undefined);
  } finally {
    routing?.dispose();
    ws?.close();
...
test('loopback lite: approval round-trip → respond rpc reaches the lite → complete', async () => {
  const server = http.createServer();
  const registry = createRemoteAgentsRegistry();
  let wss: WebSocketServer | undefined;
  let routing: ReturnType<typeof createRemoteRouting> | undefined;
