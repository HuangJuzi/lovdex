...
type QoderHistoryMessagesResult =
  | AnyRecord[]
  | {
    messages: AnyRecord[];
    total: number;
...
/**
 * Encodes a project path the same way Qoder names its project directories:
 * every `/` is replaced with `-` (e.g. `/mnt/b/workdir/gitlab/backend` becomes
 * `-mnt-b-workdir-gitlab-backend`). Used to reconstruct the on-disk transcript
 * path when the synchronizer has not yet recorded a `jsonl_path`.
 */
function encodeQoderProjectDir(projectPath: string): string {
  return projectPath.replace(/\//g, '-');
}

/**
 * Resolves the transcript path for one Qoder session.
 *
 * Prefers the `jsonl_path` recorded by the session synchronizer; when absent
 * (e.g. an app-created session that has not been synchronized yet), the path is
 * reconstructed from the project path and provider session id.
 */
function resolveQoderHistoryPath(
  session: { jsonl_path: string | null; project_path: string | null } | null | undefined,
  ...
  return path.join(
    ...
    '.qoder',
    'projects',
    encodeQoderProjectDir(projectPath),
...
async function getSessionMessages(
  ...
  limit: number | null,
  offset: number,
): Promise<QoderHistoryMessagesResult> {
  try {
    ...
    const jsonLPath = resolveQoderHistoryPath(session, providerSessionId);
...
/**
 * Qoder writes a mix of truly internal transcript rows and "UI-hidden" local
 ...
 *
 * Skill bodies belong in the first group. When a skill is invoked, Qoder
...
/**
 * Qoder wraps local slash-command metadata in lightweight XML-like tags inside
...
/**
 * Converts Qoder's hidden local command wrapper into structured metadata.
 *
 ...
 */
function parseLocalCommandPayload(content: string): QoderLocalCommandPayload | null {
 ...
 */
function buildLocalCommandDisplayText(payload: QoderLocalCommandPayload): string {
...
/**
 * Qoder local-command stdout may contain ANSI styling codes because it was
...
export class QoderSessionsProvider implements IProviderSessions {
  ...
  normalizeMessage(rawMessage: unknown, sessionId: string | null): NormalizedMessage[] {
    ...
    if (raw.message?.role === 'user' && raw.message?.content && raw.isMeta !== true) {
      if (Array.isArray(raw.message.content)) {
        ...
        /**
         * Qoder stores compact summaries as synthetic "user" rows so the CLI
        ...
        /**
         * Local command stdout is also written as a "user" row in Qoder's
  ...
  /**
   * Loads Qoder JSONL history for a project/session and returns normalized
