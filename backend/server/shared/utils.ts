 ...
 * possible. If `inputPath` is already lexically under the root it is returned
 * unchanged. Otherwise, when `inputPath` resolves (via realpath) to the same
 * physical location as a path that IS under the root — found by walking the
 * immediate symlink children of the root whose target is a prefix of the
 * realpath — that under-root spelling is returned. Otherwise the original
 * string is returned unchanged.
 *
 * This collapses symlink-target spellings (e.g. `/mnt/b/workdir/foo`) and
 * `~`-relative spellings (e.g. `/home/zhijuhuang/workdir/foo`) onto a single
 * canonical `~`-relative form, so the same folder cannot be stored as two
 * different `project_path` strings (which would bypass the UNIQUE constraint
 * and surface as duplicate project entries).
 */
let workspaceSymlinkMap: { target: string; linkAbs: string }[] | null = null;
function getWorkspaceSymlinkMap(): { target: string; linkAbs: string }[] {
  if (workspaceSymlinkMap) {
  ...
  for (const entry of entries) {
    ...
    const linkAbs = path.join(WORKSPACES_ROOT, entry.name);
    let target: string;
    try {
      target = fs.realpathSync(linkAbs);
    ...
    map.push({ target, linkAbs });
  }
  workspaceSymlinkMap = map;
  return map;
}

export function normalizeToWorkspaceRoot(inputPath: string): string {
  if (!inputPath || typeof inputPath !== 'string') {
    return inputPath;
  }
  const root = WORKSPACES_ROOT;
  if (inputPath === root || inputPath.startsWith(`${root}${path.sep}`)) {
    return inputPath;
  }
  ...
  for (const { target, linkAbs } of getWorkspaceSymlinkMap()) {
    if (realInput === target || realInput.startsWith(`${target}${path.sep}`)) {
      const candidate = path.join(linkAbs, realInput.slice(target.length));
...
export async function validateWorkspacePath(requestedPath: string): Promise<WorkspacePathValidationResult> {
  try {
    ...
    // Containment is judged on the requested (lexical) path, not on the
    // realpath. This lets a symlinked subtree under the root be used
    // project-by-project without broadening the root: a path like
    // ~/workdir/oem/cli (where ~/workdir -> /mnt/b/workdir) is lexically under
    // ~ even though its realpath is not, and the symlink leaf ~/workdir itself
    // is accepted the same way. `path.resolve` has already collapsed any `..`
    // traversal, so the lexical path is safe to compare against the root. The
    // persisted project path is the lexical path, so it stays under the root.
    const linkPath = normalizeProjectPath(absolutePath);
    if (
      !linkPath.startsWith(`${resolvedWorkspaceRoot}${path.sep}`)
      && linkPath !== resolvedWorkspaceRoot
    ) {
      return {
        valid: false,
