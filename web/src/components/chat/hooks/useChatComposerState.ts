...
const getNotificationSessionSummary = (
  ...
  const sessionSummary = resolveSessionTitle(selectedSession);
...
export function useChatComposerState({
  ...
  const handleSubmit = useCallback(
    async (
      ...
      if (isLoading) {
        dbg('bail isLoading -> stash queued draft');
      ...
      // Commands flagged `metadata.forwardToProvider` (e.g. /clear, /compact)
      // are NOT intercepted — they fall through to a normal chat.send so the
      // provider receives the raw command text and interprets it natively.
      ...
      dbg(`preSend sid=${targetSessionId} len=${messageContent.length} img=${uploadedImages.length}`);
      sendMessage({
        type: 'chat.send',
