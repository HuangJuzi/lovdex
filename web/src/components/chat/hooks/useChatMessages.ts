...
import { classifyAutoApproveNotice, interactionNoticeCopy } from '../utils/autoApproveDeny';
  ...
  for (const msg of messages) {
    ...
    switch (msg.kind) {
      ...
      case 'permission_auto': {
        ...
        // 三选一按 `kind` 分流，不再另立 `behavior === 'deny'` 这个同源派生轴：
        // deny ⇒ kind ∈ {interaction, blocked}，allow ⇒ undefined，两个轴等价。
        const content = kind === 'interaction'
          ? interactionNoticeCopy(msg.toolName)
          : kind === 'blocked'
