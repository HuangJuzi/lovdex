...
export const SubagentContainer: React.FC<SubagentContainerProps> = ({
  ...
  return (
    <div className="my-1 border-l-2 border-l-chart-6 py-0.5 pl-3">
      <CollapsibleSection
        title={title}
        toolName="Task"
        open={false}
      >
        ...
        {currentTool && !isComplete && (
          <div className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
            <span className="h-1.5 w-1.5 flex-shrink-0 animate-pulse rounded-full bg-chart-6" />
            ...
            <CollapsibleContent>
              <div className="mt-1 space-y-0.5 border-l border-border pl-3">
                {childTools.map((child, index) => (
                  <div key={child.toolId} className="flex items-center gap-1.5 text-2xs text-muted-foreground">
          ...
          <div className="mt-2 text-xs text-muted-foreground">
            {(() => {
              ...
              return typeof content === 'string' ? (
                ...
                <pre className="line-clamp-6 whitespace-pre-wrap break-words font-mono text-2xs">
