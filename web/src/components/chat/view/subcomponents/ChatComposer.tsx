...
export default function ChatComposer({
  ...
  // Simplified edition: voice input removed.
  ...
  const voiceState = 'idle' as string;
  const voiceToggle = useCallback(() => {}, []);
  const voiceStop = useCallback((_opts: unknown) => {}, []);
  ...
  return (
    <div className="chat-composer-shell relative flex-shrink-0 px-2 pb-2 pt-0 sm:px-4 sm:pb-4 md:px-4 md:pb-6">
      {!hasPendingPermissions && (
        <div className="pointer-events-none absolute bottom-full left-1/2 z-10 w-[calc(100%-1rem)] -translate-x-1/2 translate-y-px bg-transparent sm:w-[calc(100%-2rem)]">
