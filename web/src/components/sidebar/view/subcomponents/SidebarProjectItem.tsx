...
export default function SidebarProjectItem({
  ...
  const remoteMarker = remoteHostName ? (
    <span
      role="img"
      className="inline-flex flex-shrink-0 items-center gap-0.5 rounded bg-chart-4/10 px-1 py-0.5 text-3xs font-medium text-chart-4"
  ...
  return (
    ...
    >
      {/* 项目头吸顶：sticky 的包含块是本元素的父级（挂了 itemRef 的那个最外层
          div），而父级同时包含下面的 SidebarProjectSessions，所以项目头会一直
          粘到本项目会话列表结束，再被下一个项目行顶走。

          不要把它挪到项目行按钮上 —— 那样包含块只剩项目行自身高度，吸顶会
          静默失效。也不要在 itemRef 与本元素之间插入任何 overflow-hidden /
